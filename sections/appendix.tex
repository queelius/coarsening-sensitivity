...
\begin{equation}
  p_\delta(r; \theta) \;\propto\; \int_{c(r)} g(y \mid \theta)\, e^{\delta h(r,y)}\,
    d\nu(y),
  \label{eq:app-tilted}
\end{equation}
...
B(\theta^\star) + O(\delta^2)$, which is \eqref{eq:Bdelta}. The supremum is attained
when $\bar h$ is proportional to the $L^2(P_0)$ projection of $s$ onto the
candidate-set-measurable functions, the worst-case tilt aligned with the score.

Finally, suppose the report law is a regular exponential family with natural
statistic $\T$ and natural parameter $\eta(\theta)$, and the tilt is a function of
$\T$. Then $e^{\delta h}$ shifts the natural parameter, $p_\delta$ is the same
exponential family at $\eta(\theta) + \delta h$, and the pseudo-true $\theta_\delta$
...
(signal $\gamma$, noise scale $\sigma$), and per-singleton influence-function
variance at most $\sigma^2$.
...
$m_{v^\star} = \Omega(\sigma^2/(\gamma^2 \eps^2))$. The $r$ orthogonal confounded
directions impose this independently, so $n_{\mathrm{s}} = \Omega(r \sigma^2 /
