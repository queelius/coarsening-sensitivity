...
$\Info^{-1}\Cov(s,h)$ is read off the known mechanism. The engineered location-family
tilt $h(m) = (m - \mu)/c$ is unbounded in $m$, so the $\|h\|_\infty \le 1$
normalization of \cref{def:tilt} is taken on the effective support; the bias identity
\eqref{eq:bias} needs only a finite score-tilt covariance and applies regardless,
while the supremum bound \eqref{eq:Bdelta} is the statement for the bounded-tilt
regime. For \cref{thm:restoration} the
