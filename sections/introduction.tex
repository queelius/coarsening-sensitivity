...
\item \textbf{A sensitivity bound} (\cref{thm:sensitivity}, \cref{sec:sensitivity}).
  ...
  partially identified over a $\delta$-ball that contracts to a point as
  $\delta \to 0$. In a regular exponential family the leading term is exact at
  every $\delta$ along curved directions; the marginal-fit check that
  \citet{towell2026synthesis} shows is blind to bias under C2 is exactly the
  quantity the bound controls.
...
\item \textbf{Six domain instances} (\cref{sec:instances}) and a cross-domain
  ...
  $r/\gamma^2$ rate holds across reliability, single-cell and spatial genomics,
  differential privacy, weak supervision, and phenotyping.
\end{enumerate}
